...
ns <- 9915 # 1000, 9915
ex_settings <- "401k" # c("normal", "mixture2", "mixture3", "logistic", "t4"), "401k"
f_settings <- "additive" # c("plm", "additive", "interaction")
...
with_progress( {
  ...
  sim_res <- future_apply(sim_df, MARGIN=1, future.seed=TRUE, future.scheduling = Inf, simplify=FALSE, FUN = function(x) {
    ...
    out <- suppressWarnings(compare(n=n,
                                    ex_setting=ex,
                                    f_setting=f))
    return(c("sample_theta"=out$sample_theta, out$est, out$se))
  })
})
...
write.csv(sim_res_df, "simulation_results_401k_add.csv", row.names=FALSE)
